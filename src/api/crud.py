...
# Session CRUD
def create_session(db: Session, operator_id: int, device_id: int = None):
...
def get_active_session_by_operator(db, operator_id):
    """
    Trả về session đang active (status='active') của operator, nếu có.
    """
    return db.query(DbSession).filter_by(operator_id=operator_id, status="active").first()

# Thêm hàm mới để cập nhật device_id cho session

def update_session_device(db: Session, session_id: int, device_id: int):
    """Cập nhật device_id cho session dựa vào ID"""
...
def get_all_sessions(db: Session) -> List[models.Session]:
    ...
    ).order_by(desc(models.Session.started_at)).all()
...
def get_command_logs(
    ...
    query = db.query(models.Log).options(
        joinedload(models.Log.user), 
        joinedload(models.Log.device)
    )

    if operator_id:
        query = query.filter(models.Log.user_id == operator_id)
    if device_id:
        query = query.filter(models.Log.device_id == device_id)
    if start_date:
        query = query.filter(models.Log.timestamp >= start_date)
    if end_date:
        query = query.filter(models.Log.timestamp < (end_date + timedelta(days=1))) # Inclusive of the end date

    # Sorting
    if sort_by == "operator_username":
        query = query.join(models.Log.user, isouter=True) # Use outer join to not lose logs if user is deleted
        sort_column = models.User.username
    elif sort_by == "device_ip_address":
        query = query.join(models.Log.device, isouter=True) # Use outer join
        sort_column = models.Device.ip_address
    elif hasattr(models.Log, sort_by):
        sort_column = getattr(models.Log, sort_by)
    else: 
        sort_column = models.Log.timestamp # Default sort
...
def get_command_logs_count(
    ...
    if operator_id:
        # Need to join if filtering on related table fields, but here user_id is direct
        query = query.filter(models.Log.user_id == operator_id)
    if device_id:
        query = query.filter(models.Log.device_id == device_id)
    if start_date:
        query = query.filter(models.Log.timestamp >= start_date)
    if end_date:
        query = query.filter(models.Log.timestamp < (end_date + timedelta(days=1)))
    
    count = query.scalar()
    return count if count is not None else 0

def get_all_devices_for_filter(db: Session) -> List[models.Device]:
    return db.query(models.Device).order_by(models.Device.ip_address).all()
...
def end_operator_sessions(db, operator_id):
    from datetime import datetime
    sessions = db.query(DbSession).filter_by(operator_id=operator_id, status="active").all()
    for session in sessions:
        session.status = "completed"
        session.ended_at = func.now()
    db.commit()

def get_unique_devices_from_logs(db: Session) -> List[models.Device]:
    """
    Lấy danh sách các thiết bị duy nhất có ít nhất một bản ghi trong bảng logs.
    """
    # Lấy các device_id duy nhất từ bảng Log, bỏ qua các giá trị NULL
    logged_device_ids_query = db.query(distinct(models.Log.device_id)).filter(models.Log.device_id.isnot(None))
    
    # Chuyển kết quả truy vấn thành một danh sách các ID
    logged_device_ids = [item[0] for item in logged_device_ids_query.all() if item[0] is not None]

    if not logged_device_ids:
        return []

    # Lấy thông tin chi tiết của các thiết bị dựa trên các ID đã thu thập
