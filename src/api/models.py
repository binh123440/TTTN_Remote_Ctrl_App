...
class Session(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    operator_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    device_id = Column(Integer, ForeignKey("devices.id", ondelete="CASCADE"), nullable=True)  # Đổi nullable=False thành nullable=True
    ...
    device = relationship("Device", back_populates="sessions", foreign_keys=[device_id])